...
19. Added explicit strategy-mode gating for ML availability (`ML` / `BASELINE` / `HOLD_ONLY`): if `USE_ML=1` and ML is unavailable, default is `HOLD_ONLY` (no new buys) unless `ALLOW_BASELINE_TRADING=1`; mode is emitted in run metadata and workflow health summary (`stock_screener/config.py`, `stock_screener/pipeline/daily.py`, `.github/workflows/daily-stock-screener.yml`, `tests/test_pipeline_smoke.py`).
20. State transition persistence hardened: post-exit persistence is now fail-stop (no best-effort continuation), position-changing actions are appended to `screener_portfolio_state.json.events.jsonl`, and state can rebuild from events when primary/backup JSON are unreadable (`stock_screener/portfolio/state.py`, `stock_screener/pipeline/daily.py`, `stock_screener/portfolio/manager.py`, `.github/workflows/daily-stock-screener.yml`, `.github/workflows/reset-portfolio-state.yml`, `tests/test_portfolio_state.py`, `tests/test_portfolio_manager.py`, `tests/test_pipeline_smoke.py`).
21. Rotation/exit conflict controls added: `build_trade_plan` now enforces a single per-ticker decision map (`EXIT`/`REDUCE`/`HOLD`/`INCREASE`/`ENTER`), blocks rotation for tickers pre-decided by exit pass (`blocked_buys` tickers still open), and applies `rotation_cooldown_days` to prevent immediate post-entry rotation churn unless risk exits already fired in `apply_exits` (`stock_screener/config.py`, `stock_screener/portfolio/manager.py`, `stock_screener/pipeline/daily.py`, `.github/workflows/daily-stock-screener.yml`, `tests/test_portfolio_manager.py`).
22. Rebalance exposure policy is now explicit and telemetry-backed: pipeline tracks `gross_exposure`, `net_exposure`, and `cash_weight` before/after rebalance controls, defaults to `allow_cash_no_upscale` (keep cash; only downscale infeasible gross), and supports optional `normalize_to_target_gross` policy. This avoids implicit post-hoc scaling drift while keeping unified-optimizer outputs unchanged unless infeasible (`stock_screener/config.py`, `stock_screener/pipeline/daily.py`, `.github/workflows/daily-stock-screener.yml`, `tests/test_rebalance_hysteresis_dynamic.py`, `tests/test_pipeline_smoke.py`).
23. Rebalance projection audit added: pipeline now records deterministic gate diagnostics (reason-code counts, dropped tickers, explicit notional drops), and reports weight drift (`L1`/`L2`) for both `pre_rebalance -> final` and `optimizer -> final` (when unified optimizer is enabled). Daily text report and workflow health summary now surface these deltas so post-optimizer feasibility projections are observable instead of silent (`stock_screener/pipeline/daily.py`, `stock_screener/reporting/render.py`, `.github/workflows/daily-stock-screener.yml`, `tests/test_rebalance_hysteresis_dynamic.py`, `tests/test_pipeline_smoke.py`).
24. Entry threshold guardrails hardened: dynamic relaxation is now gated by cross-sectional separation (top-decile spread checks), stress signals (vol-regime/breadth) tighten thresholds, and optional stress-triggered `HOLD_ONLY` escalation can freeze new entries (`stock_screener/config.py`, `stock_screener/pipeline/daily.py`, `.github/workflows/daily-stock-screener.yml`, `tests/test_daily_dynamic_sizing.py`, `tests/test_dynamic_sizing_model_ic.py`, `tests/test_pipeline_smoke.py`).
25. `ret_per_day` signal hardened: predicted peak days are clamped to configurable bounds, denominator smoothing (`+k`) prevents small-day blowups, and per-run distribution stats/shift alerts are logged against prior-run metadata so rank-instability drift is visible in operations telemetry (`stock_screener/config.py`, `stock_screener/pipeline/daily.py`, `.github/workflows/daily-stock-screener.yml`, `tests/test_daily_dynamic_sizing.py`, `tests/test_dynamic_sizing_model_ic.py`).
